...
> [!NOTE]
> **与原生 `enum` 的选型原则**：
> - **优先标准 `enum`**：若字段取值为常规枚举且选项名称无需额外 UI 友好别名（如纯分辨率 `1080P`、标准模型标识等），直接使用 OpenAPI 标准 `enum` 即可，**无需添加冗余的 `x-options`**。
> - **仅在需要 UI 增强时使用**：仅当选项需要复杂的展示名或需要 `group` 分组时才添加 `x-options`。
...
1. **标准规范优先**：
   - 优先使用 OpenAPI 3.0 原生字段（如 `enum`, `default`, `minimum`, `maximum` 等）表达参数约束与离散值范围；仅在原生字段无法满足交互逻辑（如动态联动、复杂别名/分组）时引入 `x-` 扩展。
2. **命名规范**：
